...
REQ=ROOT/'docs/preproduction/catalog-sprint/chat-decor-recovery-1-2-20260921.json'
REPORT=ROOT/'docs/preproduction/catalog-sprint/chat-decor-recovery-1-2-result.json'
ORIG=ROOT/'docs/preproduction/catalog-sprint/recovered-originals/decor-1-2-20260921'
EXPECTED=['decor-1','decor-2']; BRANCH='screenshot-match-preproduction'; MAX=12*1024*1024
...
def main():
 ...
 if REPORT.exists():
  print('Decor 1-2 already staged; no duplicate recovery'); return
 rows=[]
 for x in req['items']:
  real=meta[x['id']]
  ...
  row.update(ci); row.update({'producer':'09','transportHelper':'CHAT','independentReviewer':'14','repositoryPath':str(cp.relative_to(ROOT)),'assetPath':'/'+str(cp.relative_to(ROOT/'public')),'assetId':x['assetId'],'status':'READY_FOR_REVIEW_14','readback':'PASS','original':{**si,'repositoryPath':str(op.relative_to(ROOT))}})
  rows.append(row); print(row['id'],row['dimensions'],row['gitBlobSha'])
 if len({r['sha256'] for r in rows})!=2: raise ValueError('duplicate content')
 REPORT.write_text(json.dumps({'schemaVersion':1,'batchId':req['batchId'],'status':'STAGED_READY_FOR_INDEPENDENT_REVIEW_14','sourceHead':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'workflowRunId':os.environ.get('GITHUB_RUN_ID'),'producer':'09','transportHelper':'CHAT','independentReviewer':'14','canonicalWriter':'08','newGenerations':0,'storedCandidateCount':2,'storedOriginalCount':2,'duplicateCandidateContent':0,'items':rows,'checks':{'metadata':'PASS_2_OF_2','exactByteReadback':'PASS_4_OF_4','candidateDecode':'PASS_2_OF_2','independentVisualReview':'PENDING_14'},'freeze':{'replitTouched':False,'flootTouched':False,'mainTouched':False,'canonicalMappingsTouched':False,'gameplayChanged':False,'playerDataTouched':False}},indent=2)+'\n')
if __name__=='__main__':
 try: main()
 except Exception as e:
  print('Decor recovery FAILED: '+str(e),file=sys.stderr); raise
