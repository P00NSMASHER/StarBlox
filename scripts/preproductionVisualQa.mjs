...
async function desktopGeometryChecks(page,screen){
  ...
  if(screen.name==='Store'){
    ...
    const fallback=await page.locator('.marketPage.sbStoreMatch .sbStoreFallbackArt:visible').count().catch(()=>0);record({viewport:vp,screen:'Store',check:'visible-final-art-fallbacks',status:fallback===0?'PASS':'FAIL',releaseBlocking:fallback>0,message:`Visible generic fallback art instances in current default category: ${fallback}.`});
