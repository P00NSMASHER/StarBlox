"""One-batch byte transport. Never edits images, catalog mappings, or player state."""
...
REQUEST = ROOT / 'docs/preproduction/catalog-sprint/chat-lighting-recovery-20260921.json'
REPORT = ROOT / 'docs/preproduction/catalog-sprint/chat-lighting-recovery-result.json'
ORIGINALS = ROOT / 'docs/preproduction/catalog-sprint/recovered-originals/lighting-5-8-20260921'
EXPECTED_IDS = ['lighting-5', 'lighting-6', 'lighting-7', 'lighting-8']
...
def fetch_bytes(url: str) -> bytes:
    ...
    if parsed.scheme != 'https' or parsed.hostname != 'at.adobe.com':
        raise ValueError('Only the four connector-returned Adobe HTTPS links are allowed')
...
def image_info(data: bytes) -> dict:
    ...
    with Image.open(io.BytesIO(data)) as image:
        ...
        if all(lo == hi for lo, hi in (extrema if isinstance(extrema[0], tuple) else [extrema])):
...
def main() -> None:
    ...
    if [item['id'] for item in request['items']] != EXPECTED_IDS:
        raise ValueError('This importer accepts only Lighting 5–8')
...
if __name__ == '__main__':
    try:
        ...
        print('Lighting intake FAILED: ' + str(error), file=sys.stderr)
        raise
