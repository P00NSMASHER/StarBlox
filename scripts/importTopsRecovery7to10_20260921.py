...
REQ=ROOT/'docs/preproduction/catalog-sprint/chat-tops-recovery-7-10-20260921.json'
REPORT=ROOT/'docs/preproduction/catalog-sprint/chat-tops-recovery-7-10-result.json'
ORIG=ROOT/'docs/preproduction/catalog-sprint/recovered-originals/tops-7-10-20260921'
EXPECTED=['tops-7','tops-8','tops-9','tops-10']; BRANCH='screenshot-match-preproduction'; MAX=16*1024*1024
...
def main():
 ...
 if REPORT.exists():
  print('Tops 7-10 already staged; no duplicate recovery'); return
 rows=[]
 for x in req['items']:
  real=meta[x['id']]
  ...
  row.update(ci); row.update({'producer':'09','transportHelper':'CHAT','independentReviewer':'01','repositoryPath':str(cp.relative_to(ROOT)),'assetPath':'/'+str(cp.relative_to(ROOT/'public')),'assetId':x['assetId'],'status':'READY_FOR_REVIEW_01','readback':'PASS','original':{**si,'repositoryPath':str(op.relative_to(ROOT))}})
