...
function replacementFileId(file) {
  return file.match(/^([a-z]+-\d+)-.+\.(?:svg|png|jpe?g|webp)$/i)?.[1] ?? null;
}
...
function collectReplacementCandidates() {
  ...
  for (const lanePath of producerLanes) {
    ...
    for (const o of allObjects(lane)) {
      ...
      byKey.set(`${o.id}:${sha}`, {id:o.id,name:o.name ?? o.id,tier:o.tier ?? null,theme:o.theme ?? null,producerLane:lanePath,discovery:'lane-reviewable',repositoryPath,blobSha:sha});
    }
  }
  ...
  if (fs.existsSync(root)) {
    for (const file of fs.readdirSync(root).sort()) {
      ...
      byKey.set(`${id}:${sha}`, {id,name:meta.name ?? id,tier:meta.tier ?? null,theme:meta.theme ?? null,producerLane:meta.producerLane ?? null,discovery:'versioned-file-scan',repositoryPath,blobSha:sha});
    }
  }
...
async function renderSet(browser, setName, items, report) {
  ...
  try {
    ...
    for (const image of setReport.contactSheetImageStatus) {
      if (!image.naturalWidth) setReport.contactSheetErrors.push(`contact-sheet decode failed: ${image.src}`);
    }
    await page.screenshot({path:path.join(out, `${setName}-contact-sheet.png`), fullPage:true, timeout:15000});
  } catch (error) {
    setReport.contactSheetErrors.push(String(error));
  } finally {
    await page.close();
  }
  ...
  for (const item of items) {
    ...
    try {
      ...
      const imageState = (await settleImages(p))[0];
...
const report = {sourceHead:process.env.GITHUB_SHA,generatedAt:new Date().toISOString(),sets:{},errors:[]};
...
const failures = [
  ...report.errors,
  ...Object.entries(report.sets).flatMap(([name,set]) => [
