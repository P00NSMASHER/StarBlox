...
REQ=ROOT/'docs/preproduction/catalog-sprint/chat-rugs-recovery-1-4-20260921.json'
REPORT=ROOT/'docs/preproduction/catalog-sprint/chat-rugs-recovery-1-4-result.json'
ORIG=ROOT/'docs/preproduction/catalog-sprint/recovered-originals/rugs-1-4-20260921'
EXPECTED=['rugs-1','rugs-2','rugs-3','rugs-4']; BRANCH='screenshot-match-preproduction'; MAX=12*1024*1024
...
def write(path,data):
 ...
 if path.read_bytes()!=data: raise ValueError('readback fail')
def main():
 ...
 if REPORT.exists():
  print('Rugs 1-4 report already exists; no duplicate recovery'); return
 rows=[]
 for x in req['items']:
  real=meta[x['id']]
  ...
  if ci['format']!='JPEG': raise ValueError('candidate rendition must be JPEG')
...
if __name__=='__main__':
 try: main()
  ...
  print('Rugs recovery FAILED: '+str(e),file=sys.stderr); raise
