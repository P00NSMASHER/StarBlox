...
import { assertDailyPublishable, inspectDailyBundleArtifact } from './dailyCertification.js';
  ...
  for(const [date,entry] of Object.entries(registry.dates || {})){
    ...
    for(let index=0;index<entry.versions.length;index++){
      ...
      if(release.artifact?.status !== 'certified' || release.artifact?.bundle?.compatibility?.ok !== true){
        ...
        const inspection=inspectDailyBundleArtifact(release.artifact);
        if(!inspection.ok){
          errors.push(
            'release artifact failed full certification inspection: ' +
            release.releaseId + ' (' +
            inspection.failures.map(item => item.id).join(', ') + ')'
          );
        }
      }
      if(release.frozenAt != null){
        try{ cleanTimestamp(release.frozenAt,'release frozenAt'); }
        catch(error){ errors.push(error.message + ': ' + release.releaseId); }
      }
      if(expectedVersion > 1 && !String(release.reason || '').trim()){
        errors.push('versioned release is missing a reason: ' + release.releaseId);
      }
...
export function freezeCertifiedDaily(registry,artifact,{
  ...
  const inspection=inspectDailyBundleArtifact(artifact);
  if(!inspection.ok){
    throw new Error(
      'certified Daily failed full certification inspection: ' +
      inspection.failures.map(item => item.id).join(', ')
    );
  }
...
export function buildDailyReleaseManifest(registry){
  ...
  const entries=dates.map(date => {
    ...
    return {
      ...
      artifactHash:preferred.artifactHash
    };
  });
