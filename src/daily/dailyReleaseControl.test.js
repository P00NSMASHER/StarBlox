...
describe('Step 16: deterministic Daily freeze, fallback and versioning', () => {
  ...
  it('builds a sorted manifest and falls back only to an earlier frozen date without regenerating', async () => {
    ...
    expect(manifest.entries.every(entry => /^fnv1a32:[a-f0-9]{8}$/.test(entry.artifactHash))).toBe(true);
...
describe('Step 16: certified release trust boundary', () => {
  it('rejects superficially certified artifacts whose internal certificate no longer verifies', async () => {
    ...
    tampered.questionSet[0].question.prompt+=' tampered after certification';

    expect(tampered.status).toBe('certified');
    expect(tampered.bundle.compatibility.ok).toBe(true);
    expect(() => freezeCertifiedDaily(
      createDailyReleaseRegistry(),
      tampered
    )).toThrow(/full certification inspection/);
  });
});
...
describe('Step 17: Daily schedule and activation control plane', () => {
  it('derives a deterministic activation slot from date + release identity', async () => {
    ...
    expect(() => deterministicActivationAt('2026-10-20',release.releaseId,{
      startHourUtc:23,
      startMinuteUtc:30,
      windowMinutes:60,
      slotMinutes:10
    })).toThrow(/crosses the UTC calendar day/);
  });
  ...
  it('rejects a schedule timestamp that falls on a different UTC calendar date', async () => {
    ...
    })).toThrow(/same UTC calendar date/);
  });

  it('detects control-state tampering even when the outer control hash is recomputed', async () => {
    const artifact=await certified('2026-10-29');
    const frozen=freezeCertifiedDaily(createDailyReleaseRegistry(),artifact);
    let control=scheduleDailyRelease(createDailyControlPlane(),frozen.registry,{
      date:'2026-10-29',
      releaseId:frozen.release.releaseId,
      scheduledAt:'2026-10-29T00:05:00Z'
    }).control;
    control=activateDueDaily(control,frozen.registry,{
      date:'2026-10-29',
      now:'2026-10-29T00:05:00Z'
    }).control;

    const tampered=JSON.parse(JSON.stringify(control));
    delete tampered.activeByDate['2026-10-29'];
    tampered.controlHash=stableHash({
      schemaVersion:tampered.schemaVersion,
      controlVersion:tampered.controlVersion,
      schedules:tampered.schedules,
      activeByDate:tampered.activeByDate,
      history:tampered.history
    });

    const validation=verifyDailyControlPlane(tampered,frozen.registry);
    expect(validation.ok).toBe(false);
    expect(validation.errors.join(' ')).toMatch(/missing from activeByDate/);
  });

  it('detects history tampering even if the outer state hash is recomputed', async () => {
    const artifact=await certified('2026-10-30');
    const frozen=freezeCertifiedDaily(createDailyReleaseRegistry(),artifact);
    let control=scheduleDailyRelease(createDailyControlPlane(),frozen.registry,{
      date:'2026-10-30',
      releaseId:frozen.release.releaseId,
      scheduledAt:'2026-10-30T00:05:00Z'
    }).control;

    const tampered=JSON.parse(JSON.stringify(control));
    tampered.history[0].releaseId='daily-2026-10-30@forged';
    tampered.controlHash=stableHash({
      schemaVersion:tampered.schemaVersion,
      controlVersion:tampered.controlVersion,
      schedules:tampered.schedules,
    ...
    const validation=verifyDailyControlPlane(tampered,frozen.registry);
    expect(validation.ok).toBe(false);
    expect(validation.errors.join(' ')).toMatch(/history event hash mismatch/);
  });
