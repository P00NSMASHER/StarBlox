...
describe('StarBlox upstream implementation provenance', () => {
  ...
  it('pins the deterministic gameplay source used by the next implementation step', () => {
    ...
    expect(UPSTREAM_PROVENANCE.fsrsRsModelV6.path).toBe('src/model_v6.rs');
    expect(UPSTREAM_PROVENANCE.fsrsRsInference.path).toBe('src/inference.rs');
    expect(UPSTREAM_PROVENANCE.adaptiveIrtEngine.path).toBe('src/irt.py');
    expect(UPSTREAM_PROVENANCE.adaptiveIrtTests.path).toBe('tests/test_irt.py');
  });
});
