...
freezeCertifiedDaily() accepts only artifacts that satisfy assertDailyPublishable() **and** pass a fresh full inspectDailyBundleArtifact() certification inspection. A forged status/compatibility flag cannot bypass the freeze boundary.
...
- per-event history hashes and activation idempotency keys.

A later database adapter can persist this shape and use controlHash for optimistic compare-and-swap semantics. Validation also checks each history event hash and requires every triggered non-retired schedule to agree with activeByDate, so recomputing only the outer state hash cannot hide an inconsistent transition history.
...
- requires scheduledAt to remain on the same UTC calendar date as the Daily;
- rejects deterministic activation windows that would cross into the next UTC day.
